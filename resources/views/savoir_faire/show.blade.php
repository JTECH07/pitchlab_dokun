    ...
    <main class="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 w-full">
        ...
        <div class="mb-16">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
                ...
                <a href="{{ route('artisans.show', $artisan->id) }}" class="group bg-white rounded-2xl overflow-hidden shadow-sm ring-1 ring-gray-100 hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 block">
                    <div class="relative h-56 overflow-hidden">
                        ...
                        <div class="absolute top-4 left-4 flex items-center gap-1.5 bg-white/95 backdrop-blur px-3 py-1.5 rounded-full text-xs font-bold text-dokun-green shadow">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                            {{ $artisan->address }}
                        </div>
                    </div>
                    <div class="p-6">
                        <h3 class="serif text-xl text-dokun-charcoal mb-1 group-hover:text-dokun-green transition-colors">{{ $artisan->first_name }} {{ $artisan->last_name }}</h3>
                        <p class="text-[#C99424] text-[11px] font-bold uppercase tracking-wider mb-3">{{ $artisan->professional_name ?: __('app.welcome_master_artisan') }}</p>
                        <p class="text-gray-500 text-sm line-clamp-3 leading-relaxed mb-5">"{{ $artisan->description }}"</p>
                        <div class="flex gap-2.5 border-t border-gray-100 pt-5">
                            <span class="flex-1 text-center py-2.5 bg-dokun-green text-white rounded-xl font-bold text-sm group-hover:bg-dokun-green/90 transition-colors">{{ __('app.sf_view_profile') }}</span>
                            @if($artisan->whatsapp)
                            <span onclick="event.stopPropagation()" class="w-11 flex items-center justify-center bg-green-50 text-green-600 rounded-xl hover:bg-green-500 hover:text-white transition-colors">
                                <a href="https://wa.me/{{ str_replace(['+', ' '], '', $artisan->whatsapp) }}" target="_blank" class="w-full h-full flex items-center justify-center">
