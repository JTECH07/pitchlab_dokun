...
<style>
 ...
 /* Logo blend on dark bg */
 .nav-logo{mix-blend-mode:normal;transition:filter .3s,mix-blend-mode .3s;}
 #navbar.nav-overlay .nav-logo{filter:brightness(0) invert(1);mix-blend-mode:screen;}
 /* Links */
 #navbar .nav-link{position:relative;color:#17201D;opacity:.7;font-weight:600;transition:color .2s,opacity .2s;font-size:13px;}
 ...
 #navbar.nav-overlay .nav-link{color:#fff;opacity:.85;}
 ...
 #navbar.nav-overlay .nav-brand-sub{color:rgba(255,255,255,.8);text-shadow:0 1px 3px rgba(0,0,0,.25);}
 ...
 /* Dropdown */
 .nav-dropdown{opacity:0;transform:translateY(8px);pointer-events:none;transition:all .2s ease;}
 .nav-dropdown.show{opacity:1;transform:translateY(0);pointer-events:auto;}
</style>
<script>
...
<nav id="navbar" class="fixed w-full z-50 transition-all duration-500 {{ $transparent ? 'nav-overlay' : 'nav-scrolled' }} border-b"
 ...
 <img src="{{ url('images/dokun_logo_final.jpeg') }}" alt="ƉƆKUN" class="nav-logo w-9 h-11 rounded-xl">
 <div class="flex flex-col leading-tight">
 <span class="nav-brand font-serif text-lg tracking-wide leading-none">ƉƆKUN</span>
 <span class="nav-brand-sub text-[6px] tracking-[0.2em] font-bold uppercase">{!! str_replace(' & ', ' &<br>', __('app.brand_tagline')) !!}</span>
 </div>
 </a>
 ...
 <button @click="open=!open" class="nav-link flex items-center gap-1 px-3 py-2 rounded-lg {{ in_array($active,['savoir-faire','artisans','carte','experiences'])?'is-active':'' }}">
 {{ __('app.nav_explore') }}
 ...
 class="absolute left-0 mt-1 w-56 bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden py-1.5">
 <a href="{{ route('savoir-faire.index') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-dokun-charcoal hover:bg-dokun-ivory font-semibold transition">
 <svg class="w-4 h-4 text-dokun-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"/></svg>
 {{ __('app.nav_savoir') }}
 </a>
 <a href="{{ route('artisans.index') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-dokun-charcoal hover:bg-dokun-ivory font-semibold transition">
 <svg class="w-4 h-4 text-dokun-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
 {{ __('app.nav_artisans') }}
 </a>
 <a href="{{ route('carte') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-dokun-charcoal hover:bg-dokun-ivory font-semibold transition">
 <svg class="w-4 h-4 text-dokun-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"/></svg>
 {{ __('app.nav_carte') }}
 </a>
 <a href="{{ route('experiences.index') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-dokun-charcoal hover:bg-dokun-ivory font-semibold transition">
 <svg class="w-4 h-4 text-dokun-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
 ...
 {{-- Apprendre (dropdown) --}}
 <div class="relative" x-data="{open:false}" @mouseenter="open=true" @mouseleave="open=false">
 <button @click="open=!open" class="nav-link flex items-center gap-1 px-3 py-2 rounded-lg {{ in_array($active,['learn','play','moments'])?'is-active':'' }}">
 {{ __('app.nav_learn') }}
 <svg class="w-3 h-3 opacity-50 transition-transform" :class="open?'rotate-180':''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
 </button>
 <div x-show="open" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" x-cloak
 class="absolute left-0 mt-1 w-56 bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden py-1.5">
 <a href="{{ route('learn.index') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-dokun-charcoal hover:bg-dokun-ivory font-semibold transition">
 <svg class="w-4 h-4 text-dokun-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
 {{ __('app.nav_learn') }}
 </a>
 <a href="{{ route('play.index') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-dokun-charcoal hover:bg-dokun-ivory font-semibold transition">
 <svg class="w-4 h-4 text-dokun-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
 {{ __('app.play_badge') }}
 </a>
 <a href="{{ route('moments.index') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-dokun-charcoal hover:bg-dokun-ivory font-semibold transition">
 <svg class="w-4 h-4 text-dokun-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
 {{ __('app.nav_moments') }}
 </a>
 </div>
 </div>
 ...
 <a href="{{ route('contact') }}" class="nav-link px-3 py-2 rounded-lg {{ $active==='contact'?'is-active':'' }}">{{ __('app.nav_contact') }}</a>
 </div>
 ...
 <button type="submit" class="flex items-center px-4 py-2.5 text-sm hover:bg-dokun-ivory font-semibold w-full {{ App::getLocale()==='fr'?'bg-dokun-ivory text-dokun-green':'' }}">Fran&ccedil;ais</button>
 </form>
 <form action="{{ route('locale.switch','en') }}" method="POST">@csrf
 <button type="submit" class="flex items-center px-4 py-2.5 text-sm hover:bg-dokun-ivory font-semibold w-full {{ App::getLocale()==='en'?'bg-dokun-ivory text-dokun-green':'' }}">English</button>
 </form>
 </div>
 </div>
 ...
 @auth
 <div class="relative" x-data="{open:false}" @click.away="open=false">
 <button @click="open=!open" class="flex items-center gap-2 pl-1.5 pr-3 py-1.5 bg-dokun-green text-white rounded-full hover:bg-dokun-green/90 transition shadow text-sm font-semibold">
 ...
 <svg class="w-3 h-3 opacity-60" :class="open?'rotate-180':''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
 </button>
 ...
 <svg class="w-4 h-4 text-dokun-gold flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
 </a>
 @endauth
 ...
 <a href="{{ route('savoir-faire.index') }}" class="flex items-center gap-3 py-2.5 px-3 rounded-xl hover:bg-dokun-ivory font-semibold text-sm {{ $active==='savoir-faire'?'bg-dokun-ivory text-dokun-gold':'' }}">
 ...
 <a href="{{ route('artisans.index') }}" class="flex items-center gap-3 py-2.5 px-3 rounded-xl hover:bg-dokun-ivory font-semibold text-sm {{ $active==='artisans'?'bg-dokun-ivory text-dokun-gold':'' }}">
 ...
 <a href="{{ route('carte') }}" class="flex items-center gap-3 py-2.5 px-3 rounded-xl hover:bg-dokun-ivory font-semibold text-sm {{ $active==='carte'?'bg-dokun-ivory text-dokun-gold':'' }}">
 ...
 <a href="{{ route('experiences.index') }}" class="flex items-center gap-3 py-2.5 px-3 rounded-xl hover:bg-dokun-ivory font-semibold text-sm {{ $active==='experiences'?'bg-dokun-ivory text-dokun-gold':'' }}">
 ...
 {{-- Section : Apprendre & Jouer --}}
 <p class="text-[10px] uppercase tracking-[0.2em] font-bold text-dokun-gold px-2 mb-2">{{ __('app.nav_learn') }}</p>
 <div class="space-y-0.5 mb-5">
 <a href="{{ route('learn.index') }}" class="flex items-center gap-3 py-2.5 px-3 rounded-xl hover:bg-dokun-ivory font-semibold text-sm {{ $active==='learn'?'bg-dokun-ivory text-dokun-gold':'' }}">
 <svg class="w-5 h-5 text-dokun-gold flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
 {{ __('app.nav_learn') }}
 </a>
 <a href="{{ route('play.index') }}" class="flex items-center gap-3 py-2.5 px-3 rounded-xl hover:bg-dokun-ivory font-semibold text-sm {{ $active==='play'?'bg-dokun-ivory text-dokun-gold':'' }}">
 ...
 <a href="{{ route('moments.index') }}" class="flex items-center gap-3 py-2.5 px-3 rounded-xl hover:bg-dokun-ivory font-semibold text-sm {{ $active==='moments'?'bg-dokun-ivory text-dokun-gold':'' }}">
 ...
 {{-- Section : Infos --}}
 <div class="space-y-0.5 mb-5">
 <a href="{{ route('about') }}" class="flex items-center gap-3 py-2.5 px-3 rounded-xl hover:bg-dokun-ivory font-semibold text-sm {{ $active==='about'?'bg-dokun-ivory text-dokun-gold':'' }}">
 ...
 <a href="{{ route('contact') }}" class="flex items-center gap-3 py-2.5 px-3 rounded-xl hover:bg-dokun-ivory font-semibold text-sm {{ $active==='contact'?'bg-dokun-ivory text-dokun-gold':'' }}">
 ...
 <a href="{{ route('visitor.passport') }}" class="flex items-center gap-3 py-2.5 px-3 rounded-xl hover:bg-dokun-gold/5 font-semibold text-sm text-dokun-gold">
 <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M3 6l9-3 9 3v12l-9 3-9-3V6zm9 3v11"/></svg>
 {{ __('app.pp_title') }}
 </a>
 <a href="{{ route('dashboard') }}" class="flex items-center gap-3 py-2.5 px-3 rounded-xl hover:bg-dokun-ivory font-semibold text-sm">
 <svg class="w-5 h-5 text-dokun-gold flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M3 12l9-9 9 9M5 10v10a1 1 0 001 1h3a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1h3a1 1 0 001-1V10"/></svg>
 {{ __('app.admin_dashboard') }}
 </a>
 <form method="POST" action="{{ route('logout') }}">
 @csrf
 <button type="submit" class="flex items-center gap-3 w-full py-2.5 px-3 rounded-xl bg-red-50 text-red-600 font-bold text-sm hover:bg-red-100 transition">
 ...
 @else
 <div class="grid grid-cols-2 gap-2 mt-3 pt-4 border-t border-gray-100">
 <a href="{{ route('login') }}" class="flex items-center justify-center gap-2 w-full py-3 bg-dokun-gold text-white font-bold rounded-xl text-sm hover:bg-yellow-600 transition shadow">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1"/></svg>
 {{ __('app.nav_login') }}
 </a>
 <a href="{{ route('actor-requests.form') }}" class="flex items-center justify-center gap-2 w-full py-3 bg-dokun-green text-white font-bold rounded-xl text-sm hover:bg-dokun-green/90 transition shadow">
...
<script>
 ...
 else if(nav.classList.contains('nav-overlay')===false){nav.classList.add('nav-overlay');nav.classList.remove('nav-scrolled');}
 };
