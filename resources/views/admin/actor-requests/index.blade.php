...
<div class="max-w-7xl mx-auto">
 ...
 @php
 $pending = \App\Models\ActorRequest::where('status', 'pending')->count();
 $approved = \App\Models\ActorRequest::where('status', 'approved')->count();
 $rejected = \App\Models\ActorRequest::where('status', 'rejected')->count();
 @endphp
 ...
 @endphp
 <span class="inline-block px-2.5 py-0.5 rounded-full bg-[#C99424]/10 text-[#C99424] text-xs font-bold">{{ $roleLabels[$req->role] ?? $req->role }}</span>
 </td>
 <td class="px-5 py-4">
 ...
 @empty
 <tr><td colspan="5" class="px-5 py-10 text-center text-dokun-charcoal/40 text-sm">Aucune demande pour le moment.</td></tr>
 @endforelse
 </tbody>
 </table>
 </div>
