...
<body class="antialiased bg-dokun-ivory dark:bg-[#17201D] text-dokun-charcoal dark:text-[#e2e8e0] flex h-screen overflow-hidden" x-data="{ sidebarOpen: false }">

 <!-- Mobile overlay -->
 <div x-show="sidebarOpen" x-transition:enter="transition-opacity ease-linear duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition-opacity ease-linear duration-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" @click="sidebarOpen = false" class="fixed inset-0 bg-black/50 z-40 lg:hidden" style="display:none;"></div>

 <!-- Sidebar -->
 <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 w-64 bg-dokun-charcoal flex flex-col h-full flex-shrink-0 transition-transform duration-300 lg:translate-x-0 lg:static lg:z-auto">
 <div class="p-6 border-b border-white/10">
 <div class="flex items-center gap-3">
 <img src="{{ asset('images/dokun_logo_prim.png') }}" alt="ƉƆKUN" class="w-10 h-10 object-contain rounded shadow-lg">
 <div>
 <span class="font-serif text-white tracking-wide text-xl">ƉƆKUN</span>
 ...
 <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2H5a2 2 0 00-2 2v0M3 7l9-4 9 4"/></svg>
 ...
 <span class="truncate">Artisans</span>
 </a>
 ...
 <span class="truncate">Médias</span>
 </a>
 ...
 <span class="truncate">Carte</span>
 </a>
 ...
 <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5zm0 0h5" /></svg>
 <span class="truncate">Quartiers</span>
 </a>
 ...
 <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v2m0 14v2m9-9h-2M5 12H3m15.364 6.364l-1.414-1.414M6.05 6.05L4.636 4.636m12.728 0L15.95 6.05M6.05 17.95l-1.414 1.414M8 12a4 4 0 118 0 4 4 0 01-8 0z"/></svg>
 ...
 <span class="truncate">Voir le site public</span>
 </a>
 </nav>
 ...
 <!-- Top bar -->
 <header class="bg-white border-b border-gray-200 px-4 sm:px-8 py-4 sm:py-5 flex items-center justify-between flex-shrink-0">
 <div class="flex items-center gap-3">
 <button @click="sidebarOpen = true" class="lg:hidden p-2 -ml-2 rounded-lg hover:bg-gray-100 transition">
 <svg class="w-6 h-6 text-dokun-charcoal" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg>
 </button>
 <h1 class="text-xl sm:text-2xl font-serif text-dokun-green">@yield('page-title', 'Administration')</h1>
 </div>
 @if(session('success'))
 <div class="bg-dokun-green border border-dokun-gold/30 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-lg hidden sm:block">
 {{ session('success') }}
 </div>
 @endif
 </header>
 ...
 @if(session('success'))
 <div class="sm:hidden bg-dokun-green text-white px-4 py-2 text-sm font-bold">{{ session('success') }}</div>
 @endif
