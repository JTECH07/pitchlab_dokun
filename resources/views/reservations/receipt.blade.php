...
<body class="min-h-screen bg-gradient-to-br from-dokun-green via-dokun-green/90 to-emerald-900 dark:bg-[#17201D] dark:text-[#e2e8e0]">
...
<main class="pt-24 pb-10 px-4 flex justify-center">
 ...
 @php $shareUrl = route('reservations.receipt', $reservation->qr_code_token); $shareText = $reservation->experience_type . ' avec ' . ($reservation->artisan?->professional_name ?? $reservation->artisan?->first_name) . ' — ƉƆKUN'; @endphp
 <div class="grid grid-cols-3 gap-2">
 <a href="https://api.whatsapp.com/send?text={{ urlencode($shareText . ' ' . $shareUrl) }}" target="_blank"
 class="flex flex-col items-center gap-1 py-3 bg-[#25D366]/10 border border-[#25D366]/30 text-[#25D366] rounded-xl font-bold text-xs hover:bg-[#25D366]/20 transition justify-center">
 ...
 class="flex flex-col items-center gap-1 py-3 bg-[#1877F2]/10 border border-[#1877F2]/30 text-[#1877F2] rounded-xl font-bold text-xs hover:bg-[#1877F2]/20 transition justify-center">
 ...
 <button onclick="navigator.clipboard.writeText('{{ $shareUrl }}');this.querySelector('span').textContent='Copié !'"
 class="flex flex-col items-center gap-1 py-3 bg-dokun-gold/10 border border-dokun-gold/30 text-dokun-gold rounded-xl font-bold text-xs hover:bg-dokun-gold/20 transition justify-center">
